...
TEST_CASE("Manager opening", "[controller][class-manager][unit]")
{
  ...
  manager.register_filetype("smart-test", [](const std::smatch &) { return std::make_unique<MainController>(); });
...
{
  Manager manager;
  manager.register_filetype("test([0-9]+)", [](const std::smatch &m) -> std::unique_ptr<MainController> {
    if(m.size() <= 1) {
      return nullptr;
    }
